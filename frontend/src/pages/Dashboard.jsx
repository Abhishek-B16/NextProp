...
export default function Dashboard() {
  ...
  const defaultTab = isCustomer ? 'wishlist' : 'overview';
  ...
  const currentTabs = isCustomer ? customerTabs : ownerTabs;
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div>
          <h1 className="text-3xl font-black text-slate-100 flex items-center gap-2">
            ...
            <span>{isCustomer ? 'Customer Dashboard' : 'Property Owner Dashboard'}</span>
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Welcome back, <span className="text-slate-200 font-bold">{user?.name}</span> &bull; Manage your account preferences and activities
          </p>
        </div>
      ...
      {!isCustomer && (
        <>
          ...
          {activeTab === 'analytics' && (
            <div className="space-y-6 animate-fadeIn">
              <h2 className="text-lg font-bold text-slate-100 font-sans">Analytics & Performance Breakdown</h2>
              <AnalyticsCharts />
            </div>
          )}
